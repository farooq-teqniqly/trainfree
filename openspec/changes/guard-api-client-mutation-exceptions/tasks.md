...
- [ ] 1.1 Write a failing `ApiClientBaseTests` case asserting that a generic
...
- [ ] 1.2 Implement `ExecuteAsync<TOutcome>(Func<Task<TOutcome>> operation,
...
- [ ] 1.3 Add a `[LoggerMessage]` `Warning`-level log entry in
...
- [ ] 1.4 Add a test asserting the success path is unaffected: when `operation` completes
  without throwing, `ExecuteAsync` returns its result untouched and does not log.

## 2. Wire `ProgramsApiClient` through the guard

- [ ] 2.1 Write a failing `ProgramsApiClientTests` case per mutation method
...
- [ ] 2.2 Rewrite `CreateProgramAsync`, `RenameProgramAsync`, and `DeleteProgramAsync` to
...
- [ ] 3.1 Write a failing `SessionsApiClientTests` case per mutation method
...
- [ ] 3.2 Rewrite `CreateSessionAsync`, `RenameSessionAsync`, and `DeleteSessionAsync` to
...
- [ ] 4.1 Write a failing `PhasesApiClientTests` case per mutation method
...
- [ ] 4.2 Rewrite `CreatePhaseAsync`, `RenamePhaseAsync`, and `DeletePhaseAsync` to route
...
- [ ] 5.1 Run `dotnet test Trainfree.slnx --configuration Release` and confirm the full
...
- [ ] 5.2 Confirm no new or changed public/internal member is missing a null-guard or XML
