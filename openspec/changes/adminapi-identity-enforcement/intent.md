...
This file freezes the anchor named in `specs/admin-api-enforcement/spec.md`'s
`## Requirement coverage` header (`docs/identity/identity-intent-02-adminapi-enforcement.md`,
roadmap slice 8b) for `/opsx:gate`'s step 1 anchor-diff check, per the
`trainfree-lean` pilot's review-gate process (issue #68).

Created retroactively during the first `/opsx:gate` run on this change (no prior change
in this repo created an `intent.md`/`.anchor-snapshot.md`, so the freeze step had never
actually been wired up before now). The source doc has not been touched since commit
`070550e` (2026-09-16, "feat(identity-api): add Trainfree.IdentityApi Worker (slice 1)
(#124)"), which predates this change's own proposal commit `152250e`
("docs(openspec): propose adminapi-identity-enforcement change") -- so quoting its
current text here faithfully represents the anchor as it stood before this change was
proposed and implemented; no drift occurred in the interim that this freeze could have
otherwise missed.

Full text of `docs/identity/identity-intent-02-adminapi-enforcement.md` as of commit
`070550e`, quoted verbatim below.

---

# Trainfree Identity -- Slice 2: AdminApi enforcement

See [identity-intent.md](../../docs/identity/identity-intent.md) for shared context and
[identity-intent-01-identityapi.md](../../docs/identity/identity-intent-01-identityapi.md) for the
