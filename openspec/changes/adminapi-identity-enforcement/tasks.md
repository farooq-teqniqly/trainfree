...
- [ ] 1.1 Add a `services` binding (`IDENTITY` -> `trainfree-identity-api`) to
...
- [ ] 1.2 Add `ADMIN_INTERNAL_KEY` to `Trainfree.AdminApi/.dev.vars.example` (mirroring
...
- [ ] 1.3 Document in README.md's "Cloudflare Access application" / rollout section
...
- [ ] 2.1 Add `src/Trainfree.AdminApi/src/identity.js` exporting a function that calls
...
- [ ] 2.2 In the same module, add the `LOCAL_DEV_BYPASS` branch: when
  ...
  silent bad `userId`.
- [ ] 2.3 Add a role-check helper (`isAdministrator(identity)`) used by every enforced
  endpoint except `/api/me`. Verify with unit tests for `Administrator` and `User`
  inputs.

## 3. Enforce every data endpoint in index.js

- [ ] 3.1 Wrap every existing route handler call in `index.js`'s `fetch` (programs,
...
- [ ] 3.2 Leave `GET /api/version` and `OPTIONS` handling exactly as-is (no
...
- [ ] 3.3 Add the `GET /api/me` route: calls the enforcement module, and on
...
- [ ] 4.1 Change `createProgram(db, name)` in `programs.js` to
  ...
  the passed value.
- [ ] 4.2 Re-run existing `programs.test.js`/`index.test.js` create-program cases and
  update any call sites broken by the new parameter. Verify with `npm test` passing
  in `Trainfree.AdminApi`.

## 5. Local dev seed step

- [ ] 5.1 Confirm `Trainfree.IdentityApi/scripts/provision-identity.js` (already
  ...
  new writes.
- [ ] 5.2 Update README.md's "Local development" section: add this seed command as a
...
- [ ] 6.1 Add an `index.test.js` case asserting that with `LOCAL_DEV_BYPASS` set and
...
- [ ] 6.2 Add an `index.test.js` case asserting that with `LOCAL_DEV_BYPASS` unset and
...
- [ ] 6.3 Run the full `Trainfree.AdminApi` suite (`npm test`) and confirm all
