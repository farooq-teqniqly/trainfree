...
- [x] 1.1 Create `src/Trainfree.UI/Trainfree.UI.csproj` as a `Microsoft.NET.Sdk.Razor`
...
- [x] 1.2 Create `tests/Trainfree.UI.Tests/Trainfree.UI.Tests.csproj` mirroring
      ...
      exists.
- [x] 1.3 Register both new projects in `Trainfree.slnx` alongside the existing
...
- [x] 1.4 Extend `tests/Trainfree.ArchitectureTests/ArchitectureBoundariesTests.cs` for
...
- [x] 2.1 Write a failing bUnit test in `tests/Trainfree.UI.Tests/SkeletonBlockTests.cs`
...
- [x] 2.2 Implement `src/Trainfree.UI/SkeletonBlock.razor` with `[EditorRequired]`
...
- [x] 2.3 Add a bUnit test asserting two `SkeletonBlock` instances rendered inside a
...
- [x] 2.4 Add XML docs (`<summary>`, `<param>`) to `SkeletonBlock`'s `[Parameter]`
...
- [x] 3.1 Write a failing bUnit test in `ExercisesPageTests.cs` that holds the
...
- [x] 3.2 Add an `_isLoading` field to `Exercises.razor`, set `true` before the
      ...
      passes.
- [x] 3.3 Update the existing `OnInitialized_NoExercises_ShowsEmptyState` and
...
- [x] 4.1 Write a failing bUnit test in `PhasesPageTests.cs` mirroring 3.1 for
...
- [x] 4.2 Apply the same `_isLoading` change from 3.2 to `Phases.razor`. Verify the 4.1
      test now passes.
- [x] 4.3 Verify the existing `PhasesPageTests` suite (empty-state and load-error
...
- [x] 5.1 Write a failing bUnit test in `ProgramsPageTests.cs` that holds the program
...
- [x] 5.2 Add an `_isLoading` field to `Programs.razor`, set `true` at the start of
...
- [x] 5.3 Verify the existing `ProgramsPageTests` suite stays green: `dotnet test
      tests/Trainfree.Admin.Tests --filter FullyQualifiedName~ProgramsPageTests`.

## 6. Shimmer wiring and cross-page check

- [x] 6.1 Wrap each page's skeleton region (Programs' skeleton `tbody`, Phases' and
      ...
      scenario.
- [x] 6.2 Run `dotnet test Trainfree.slnx --configuration Release` and confirm the full
      suite is green.
- [x] 6.3 Run `dotnet csharpier check .` and fix any formatting drift introduced by the
      above.
