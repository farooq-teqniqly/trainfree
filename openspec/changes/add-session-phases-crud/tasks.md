...
- [x] 1.1 Add migration creating `session_phases` (`id` autoincrement PK, `session_phase_id`
...
- [x] 2.1 Add `generateSessionPhaseId`/`isValidSessionPhaseId` to `ids.js` with a `SPH-`
      prefix, following `generatePhaseId`/`isValidPhaseId`. Verify with a unit test in
      `ids.test.js` asserting the generated ID shape.
- [x] 2.2 Write a failing test in a new `session-phases.test.js` for `sessionExists(db,
      programId, sessionId)` (mirrors `programExists`), confirm it fails, then implement it
      in a new `session-phases.js`.
- [x] 2.3 Write a failing test for `listSessionPhases(db, sessionId)` returning
...
- [x] 2.4 Write failing tests for `createSessionPhase(db, sessionId, phaseId)`: creates a row
...
- [x] 2.5 Write a failing test for `deleteSessionPhase(db, sessionId, id)` returning whether a
...
- [x] 3.1 Add a `PhaseInUseError` to `errors.js` alongside `DuplicateNameError`.
- [x] 3.2 Write a failing test in `phases.test.js` (new file, following the pattern of the
      ...
      accordingly.
- [x] 3.3 Write a failing test confirming `deletePhase` still deletes and returns `true` for
...
- [x] 4.1 Add `handleSessionPhasesCollection`/`handleSessionPhaseResource` to `index.js`,
...
- [x] 4.2 Extend `routePrograms`'s segment-length routing to recognize
...
- [x] 4.3 Update `handlePhaseResource`'s `DELETE` branch to catch `PhaseInUseError` and
...
- [x] 4.4 Write `index.test.js` integration tests (real D1 via Miniflare, no mocking) covering:
...
- [x] 5.1 Add `SessionPhaseId` to `Trainfree.Domain/Ids/`, following `PhaseId.cs`'s shape
...
- [x] 6.1 Define `SessionPhaseSummary`, `CreateSessionPhaseOutcome`
...
- [x] 6.2 Write failing `SessionPhasesApiClient` tests (fake `HttpMessageHandler`, per
...
- [x] 6.3 Register `ISessionPhasesApiClient`/`SessionPhasesApiClient` in DI alongside the
      other API clients (wherever `ISessionsApiClient` is registered).
- [x] 6.4 Write a failing test asserting `PhasesApiClient.DeletePhaseAsync` surfaces a `409`
...
- [x] 7.1 Write failing bUnit tests (NSubstitute-faked `ISessionPhasesApiClient`) for:
      ...
      unaffected.
- [x] 7.2 Confirm each test fails, then extend `Programs.razor`/`Programs.razor.Logging.cs`
...
- [x] 7.3 Write failing bUnit tests for the `Add Phase` flow: clicking it shows a plain
...
- [x] 7.4 Write failing bUnit tests for deleting a session phase (row removed on success;
      server error shown on the row without crashing the page). Confirm each fails, then
      implement.
- [x] 7.5 Run the full bUnit suite for `Trainfree.Admin.Tests` and confirm green.

## 8. Manual verification

- [x] 8.1 Run `wrangler dev` (port 9999) and the Blazor dev server locally; in the browser,
...
- [x] 8.2 Run `dotnet test Trainfree.slnx --configuration Release` and the Worker's
