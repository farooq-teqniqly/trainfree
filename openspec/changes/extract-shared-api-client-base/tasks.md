...
- [x] 1.1 Create `src/Trainfree.ApiClients/Trainfree.ApiClients.csproj` (class library,
...
- [x] 1.2 Create sibling `tests/Trainfree.ApiClients.Tests/Trainfree.ApiClients.Tests.csproj`
...
- [x] 2.1 Write a failing test in `Trainfree.ApiClients.Tests` asserting a JSON error body
      (`{"error": "<message>"}`) is parsed to `<message>`; confirm it fails because
      `ApiClientBase` does not exist yet
- [x] 2.2 Add failing tests for: a non-JSON content-type response falling back to
      ...
      without throwing
- [x] 2.3 Add a failing test asserting the emitted log entry's category matches the
      `ILogger` instance passed in (a fake/test category), not a category fixed to the
      shared implementation type
- [x] 2.4 Implement `ApiClientBase` (public abstract class) in
...
- [x] 2.5 Implement the shared `[LoggerMessage]` declaration in
...
- [x] 3.1 Change `ProgramsApiClient` to inherit `ApiClientBase`; delete its own
...
- [x] 3.2 Repeat 3.1 for `SessionsApiClient` (and `SessionsApiClientTests`)
- [x] 3.3 Repeat 3.1 for `PhasesApiClient` (and `PhasesApiClientTests`)
- [x] 3.4 Run `dotnet test Trainfree.slnx --configuration Release` and confirm the full
      suite is green

## 4. Trainfree.ArchitectureTests

- [x] 4.1 Add `TngTech.ArchUnitNET` and `TngTech.ArchUnitNET.xUnitV3` package versions to
      `Directory.Packages.props`; verify `dotnet restore` succeeds
- [x] 4.2 Create `tests/Trainfree.ArchitectureTests/Trainfree.ArchitectureTests.csproj`
...
- [x] 4.3 Write a test asserting types in `Trainfree.Domain`, `Trainfree.Versioning`, and
...
- [x] 4.4 Verify the test actually detects a violation: temporarily add a throwaway
...
- [x] 4.5 Add a code comment or short doc note next to the rule (in the test file) that
...
- [x] 5.1 Run `dotnet format` / csharpier check across changed and new files and fix any
      formatting drift
