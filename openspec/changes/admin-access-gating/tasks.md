...
- [x] 1.1 Add `AccessCheckOutcome` (`Administrator` / `NoAccess` / `AccessCheckFailed` /
...
- [x] 1.2 Add `IAccessCheck`/`AccessCheck` calling `GET api/me` via `HttpClient`, mapping
...
- [x] 1.3 Register `IAccessCheck` and its `HttpClient` in `Program.cs` against the same
      base address the other Admin API clients use. Verify `dotnet build` succeeds and
      the app starts locally.

## 2. Root-level gate component

- [x] 2.1 Add an `AccessGate` component that wraps `<Router>`'s content: on
      ...
      resolves.
- [x] 2.2 Update `App.razor` to nest `<Router>` inside `AccessGate`, replacing the
...
- [x] 2.3 Add a `NoAccessPage` component/page shown for the `NoAccess` outcome, distinct
...
- [x] 2.4 Add an `AccessCheckErrorPage` component/page shown for the `AccessCheckFailed`
...
- [x] 2.5 Wire the `ReauthenticationRequired` outcome to force a top-level reload
...
- [x] 3.1 Run the full bUnit suite for `Trainfree.Admin.Tests` and confirm all four
...
- [x] 3.2 Manually verify against local `wrangler dev` (`LOCAL_DEV_BYPASS` synthetic
