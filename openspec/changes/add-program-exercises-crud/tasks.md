...
- [x] 1.1 Add `migrations/0013_create_program_exercises.sql` creating `program_exercises`
...
- [x] 2.1 Add `PROGRAM_EXERCISE_PREFIX = "PGX-"` and
      ...
      round-trip.
- [x] 2.2 Add program-exercise body validation to `validation.js` (or a new
...
- [x] 3.1 Write a failing `session-phases.test.js`-style `program-exercises.test.js`
...
- [x] 3.2 Add a failing test for `createProgramExercise` (Reps and Timed cases,
      defaulting `weight` to `0` and `side` to `Both`), then implement.
- [x] 3.3 Add a failing test for `updateProgramExercise` (rejects `exerciseId`/`type`
...
- [x] 3.4 Add a failing test for `deleteProgramExercise` (unconditional, 404 when not
      found under that session phase), then implement.
- [x] 3.5 Wire `GET/POST/PATCH/DELETE
...
- [x] 4.1 Write a failing test in an `exercises.test.js` (new file, mirroring
...
- [x] 4.2 Verify `index.js`'s exercise delete route maps `ExerciseInUseError` to `409`
      (mirroring the existing `PhaseInUseError` -> `409` mapping) with an
      `index.test.js` case.

## 5. Domain: program exercise types

- [x] 5.1 Add `ProgramExerciseId` to `Trainfree.Domain/Ids/` (`PGX-` prefix), following
...
- [x] 5.2 Add `RepsProgramExercise` and `TimedProgramExercise` as distinct
...
- [x] 6.1 Extend `ProgramsApiClient` (or its DTOs) with
...
- [x] 7.1 Add a `ProgramExerciseRow` row model (mirroring `SessionPhaseRow`, but with
...
- [x] 7.2 Add the `Add Exercise` action revealing the inline add-exercise form
...
- [x] 7.3 Render each program exercise row's cells per the spec's dash rules (a `0`
      ...
      normal non-zero row.
- [x] 7.4 Wire Save/Revert/Delete for program exercise rows through the dirty-row
...
- [x] 7.5 Update the `Exercises` page's delete handler to surface the Worker's new
...
- [x] 8.1 Run `dotnet test Trainfree.slnx --configuration Release` and the Worker's
