...
public sealed record VersionStamp(string Version, string Commit)
{
    ...
    public static VersionStamp FromInformationalVersion(string informationalVersion)
    {
        ...
        // Trainfree.Admin.csproj disables it for its own assembly, but this type is read via
        // typeof(VersionStamp).Assembly, i.e. this project's own compiled DLL -- which does
        // NOT disable it, so the suffix is always present here. An unstripped suffix would
        // make every load look stale rather than fail visibly.
