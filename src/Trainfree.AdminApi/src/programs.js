...
export const LIST_PROGRAMS_QUERY = `SELECT ${SELECT_COLUMNS} FROM programs ORDER BY created_at ASC, programs.id ASC`;

export async function listPrograms(db) {
    const { results } = await db.prepare(LIST_PROGRAMS_QUERY).all();
    return results;
}
